...
from PySide6.QtGui import QFont, QIcon, QPainter, QColor, QLinearGradient, QPen, QBrush, QPainterPath, QPolygon, QPixmap, QFontMetrics
import os
import sys
import cv2
...
class MapLabel(QLabel):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if route_edit_mode:
                ...
                route_history = getattr(self.main_window, 'route_history', [])
                ...
                # 添加到最后一个路线段
                is_placing_checkpoint = getattr(self.main_window, 'is_placing_checkpoint', False)
                route_segments[-1].append((original_x, original_y, is_placing_checkpoint))
    ...
    def paintEvent(self, event):
        ...
        ks_icon_path = os.path.join(sc_dir, 'ks.png')
        hua_icon_path = os.path.join(sc_dir, 'hua.png')
        ...
        ks_pixmap = QPixmap(ks_icon_path) if os.path.exists(ks_icon_path) else None
        hua_pixmap = QPixmap(hua_icon_path) if os.path.exists(hua_icon_path) else None
        ...
        if pixmap_to_draw:
            scaled_width = int(self.main_window.map_width * self.main_window.map_scale)
            scaled_height = int(self.main_window.map_height * self.main_window.map_scale)

            source_rect = QRectF(0, 0, self.main_window.map_width, self.main_window.map_height)
            target_rect = QRectF(
                self.main_window.map_offset_x,
                self.main_window.map_offset_y,
                scaled_width,
                scaled_height
            )
            ...
            if hasattr(self.main_window, 'collect_data') and self.main_window.collect_data:
                ...
                for resource_name in resources_to_show:
                    ...
                    use_ks_icon = any(special in resource_name for special in special_resources)
                    current_pixmap = ks_pixmap if use_ks_icon else hua_pixmap
                    ...
                    for point in points:
                        ...
                        if current_pixmap and not current_pixmap.isNull():
                            ...
                            marker_size = 10 * self.main_window.map_scale
                            painter.setPen(QPen(marker_color, int(3 * self.main_window.map_scale)))
        ...
        if hasattr(self.main_window, 'route_visible') and self.main_window.route_visible:
            ...
            if route_segments:
                ...
                if len(display_points) >= 2:
                    painter.setPen(QPen(QColor(34, 197, 94, 255), max(3, int(3 * self.main_window.map_scale)), Qt.SolidLine))
                    painter.setBrush(Qt.NoBrush)
                            ...
                            if segment_end - segment_start >= 2:
                                for j in range(segment_start, segment_end - 1):
                                    if display_points[j] is not None and display_points[j + 1] is not None:
                                        x1, y1, _ = display_points[j]
                                        x2, y2, _ = display_points[j + 1]
                    ...
                    for point in display_points:
                        ...
                        if is_checkpoint:
                            ...
                            painter.setPen(QPen(QColor(255, 165, 0, 255), max(2, int(2 * self.main_window.map_scale)), Qt.SolidLine))
                            ...
                            font = painter.font()
                            font.setBold(True)
                            font.setPointSize(max(10, int(10 * self.main_window.map_scale)))
                            painter.setFont(font)
                            text_size = max(16, int(16 * self.main_window.map_scale))
                            ...
                            point_radius = max(6, int(6 * self.main_window.map_scale))
                            painter.setPen(QPen(QColor(34, 197, 94, 255), max(2, int(2 * self.main_window.map_scale)), Qt.SolidLine))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._found_blind_box_bloodline = False  # 盲盒血脉是否已检测到（检测到后停止重复识别）
        self._last_box_disappeared_log_time = 0  # "幸运惊喜盒消失"日志节流
        self._blind_box_worker = None  # 盲盒血脉独立检测线程
    ...
    def _stop_recognition(self):
        ...
        # 停止盲盒血脉检测线程
        if hasattr(self, '_blind_box_worker') and self._blind_box_worker is not None:
            self._blind_box_worker.stop()
            self._blind_box_worker = None
    ...
    def start_auto_recognition(self):
        ...
        if use_roi:
            ...
            # 使用默认ROI识别模式
            from core.game_capture import ScreenshotWorker, NightmareWorker, BlindBoxBloodlineWorker
            print("🔍 启动默认识别模式")
            ...
            # 初始化盲盒血脉独立检测线程（按需启动，不影响OCR）
            self._blind_box_worker = BlindBoxBloodlineWorker(self.game_capture)
            self._blind_box_worker.bloodline_result.connect(self._on_blind_box_bloodline_result)
            self._blind_box_worker.bloodline_bonus_result.connect(self._on_blind_box_bonus_result)
    ...
    def _on_floating_nightmare_adjust(self, delta):
        ...
        try:
            ...
            logger.log(f"🔧 手动调整盲盒事件提示: {old_count} -> {active_counter.nightmare_count} ({'+' if delta > 0 else ''}{delta})")
        except Exception as e:
    ...
    def _on_floating_breakthrough_adjust(self, delta):
        ...
        try:
            ...
            logger.log(f"🔧 手动调整盲盒事件: {old_count} -> {active_counter.count} ({'+' if delta > 0 else ''}{delta})")
            
            # 同步刷新详情视图（盲盒计数模式）
            if self.content_stack.currentIndex() == 1:
                self._refresh_right_panel()
        except Exception as e:
            print(f"❌ 处理悬浮窗盲盒事件调整错误: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_recognition_result(self, result):
        ...
        try:
            ...
            # 处理识别结果 - 新逻辑：基于“幸运惊喜盒”和精灵名字的战斗流程
            if "幸运惊喜盒" in recognized_names:
                # 检测到幸运惊喜盒，标记为进入战斗
                old_battle = self.current_battle_lkwg
                self.current_battle_lkwg = "幸运惊喜盒"
                self._update_floating_current_lkwg("幸运惊喜盒")

                # 盲盒血脉：每次检测到幸运惊喜盒都重置状态并尝试启动
                self._found_blind_box_bloodline = False
                self._start_blind_box_worker()

                # 同步状态到子线程
                if hasattr(self, 'screenshot_worker') and self.screenshot_worker is not None:
                    self.screenshot_worker.current_battle_lkwg = "幸运惊喜盒"
                
                # 同步状态到ROI worker
                if hasattr(self, 'roi_worker') and self.roi_worker is not None:
                    self.roi_worker.set_current_battle("幸运惊喜盒")
                
                if old_battle != "幸运惊喜盒":
                    logger.log(f"✅ 检测到幸运惊喜盒，进入战斗")
                ...
                if active_counter:
                    ...
                    if not self._breakthrough_counted_for_current_battle:
                        ...
                        logger.log(f"✓ 盲盒事件: {active_counter.pokemon_name} | 计数: {active_counter.count}/{active_counter.target}")
                    else:
                        logger.log(f"⏭️ 跳过重复计数: {active_counter.pokemon_name}")
            elif recognized_names:
                # 识别到其他精灵名字（幸运惊喜盒消失后）
                for base_name in recognized_names:
                    if base_name in self.game_capture.evolution_manager.evolution_chains and base_name != "幸运惊喜盒":
                        # 保持战斗状态，显示精灵名
                        old_battle = self.current_battle_lkwg
                        ...
                        # 记录盲盒事件期间出现的精灵
                        active_counter = self.manager.get_active()
                        if active_counter and self.current_battle_lkwg == "幸运惊喜盒" or old_battle == "幸运惊喜盒":
                            # 在盲盒事件期间，记录出现的精灵
                ...
                if self.current_battle_lkwg:
                    # 区分“幸运惊喜盒”和“精灵名字”两种情况
                    if self.current_battle_lkwg == "幸运惊喜盒":
                        # 幸运惊喜盒消失，进入6秒等待期
                        if not should_ocr and ocr_reason == "timeout":
                            logger.log(f"⏱️ OCR超时，幸运惊喜盒消失后未检测到精灵，判定战斗结束")
                            self.current_battle_lkwg = None
                            self._update_floating_current_lkwg(None)
                            self._clear_blind_box_bloodline()
                            ...
                            import time
                            now = time.time()
                            if now - self._last_box_disappeared_log_time >= 2.0:
                                logger.log(f"⏳ 幸运惊喜盒消失，等待精灵名字出现...")
                                self._last_box_disappeared_log_time = now
                    else:
                        # 精灵名字消失，立即判定战斗结束（不等待）
    ...
    def _start_blind_box_worker(self):
        """启动盲盒血脉独立检测线程（6秒内高频检测，不影响OCR）"""
        logger.log(f"🔧 _start_blind_box_worker 被调用 (found_bloodline={self._found_blind_box_bloodline})")

        if self._found_blind_box_bloodline:
            logger.log(f"⚠️ 盲盒血脉已检测到，跳过")
            return

        if self._blind_box_worker is not None and self._blind_box_worker.isRunning():
            logger.log(f"⚠️ 盲盒检测线程已在运行中，跳过")
            return

        enabled = self.settings_manager.get("enable_blind_box_detection", False)
        if not enabled:
            logger.log(f"⚠️ 盲盒血脉检测未启用 (enable_blind_box_detection=False)")
            return

        blind_box_roi = self.settings_manager.get("blind_box_roi")
        if not blind_box_roi:
            logger.log(f"⚠️ 盲盒血脉ROI未设置 (blind_box_roi为空)")
            return

        confidence = self.settings_manager.get("blind_box_confidence", 0.6)
        detect_interval = self.settings_manager.get("blind_box_interval", 5)
        roi = (blind_box_roi["x"], blind_box_roi["y"], blind_box_roi["width"], blind_box_roi["height"])

        # 将用户设置的置信度应用到 YOLO 检测器（之前bug：从未生效）
        if hasattr(self, 'game_capture') and self.game_capture.blind_box_detector is not None:
            self.game_capture.blind_box_detector.update_conf_threshold(confidence)

        logger.log(f"🚀 启动盲盒血脉检测线程 (ROI: {blind_box_roi['x']},{blind_box_roi['y']},{blind_box_roi['width']}x{blind_box_roi['height']}, 阈值={confidence}, 帧间隔={detect_interval})")

        from core.game_capture import BlindBoxBloodlineWorker
        self._blind_box_worker = BlindBoxBloodlineWorker(self.game_capture)
        self._blind_box_worker.bloodline_result.connect(self._on_blind_box_bloodline_result)
        self._blind_box_worker.bloodline_bonus_result.connect(self._on_blind_box_bonus_result)
        self._blind_box_worker.configure(roi, confidence, timeout=8.0, detect_interval=detect_interval)
        self._blind_box_worker.start()

    def _on_blind_box_bloodline_result(self, bloodline):
        """接收盲盒血脉检测线程结果（旧信号，兼容）"""
        if bloodline:
            logger.log(f"✅ 盲盒血脉识别结果: {bloodline}")
            self._apply_blind_box_bloodline(bloodline)

    def _on_blind_box_bonus_result(self, bloodline, bonus, bl_conf, bn_conf):
        """接收盲盒血脉+加成检测结果（新MX模型信号）"""
        if bloodline and bloodline != '?':
            logger.log(f"✅ MX盲盒识别: 血脉={bloodline}({bl_conf:.0%}) 加成={bonus}({bn_conf:.0%})")
            self._apply_blind_box_bloodline_bonus(bloodline, bonus)
        else:
            logger.log(f"⚠️ MX盲盒识别失败")

    def _apply_blind_box_bloodline_bonus(self, bloodline, bonus):
        """显示血脉类型和加成到悬浮窗"""
        self._found_blind_box_bloodline = True
        if hasattr(self, 'floating_window'):
            self.floating_window.update_blind_box_bloodline(bloodline, bonus)
        self._stop_blind_box_worker()

    def _apply_blind_box_bloodline(self, bloodline):
        """显示血脉类型到悬浮窗（旧版兼容）"""
        self._found_blind_box_bloodline = True
        if hasattr(self, 'floating_window'):
            self.floating_window.update_blind_box_bloodline(bloodline)
        self._stop_blind_box_worker()

    def _stop_blind_box_worker(self):
        """停止盲盒血脉检测线程"""
        if hasattr(self, '_blind_box_worker') and self._blind_box_worker is not None:
            if self._blind_box_worker.isRunning():
                self._blind_box_worker.stop()

    def _clear_blind_box_bloodline(self):
        """清除悬浮窗上的盲盒血脉标签"""
        logger.log(f"🧹 清除盲盒血脉状态")
        self._found_blind_box_bloodline = False
        self._stop_blind_box_worker()
        if hasattr(self, 'floating_window') and hasattr(self.floating_window, 'update_blind_box_bloodline'):
            self.floating_window.update_blind_box_bloodline(None)
    ...
    def _emergency_restart_all(self):
        ...
        # 诊断盲盒工作线程
        blind_box_ok = hasattr(self, '_blind_box_worker') and self._blind_box_worker is not None
        blind_box_alive = blind_box_ok and self._blind_box_worker.isRunning()
        diagnosis.append(("盲盒血脉线程", "存活" if blind_box_alive else ("已停止" if blind_box_ok else "未创建")))
        ...
        if hasattr(self, '_blind_box_worker') and self._blind_box_worker is not None:
            try:
                self._blind_box_worker.stop()
                logger.log("  ✅ 盲盒血脉线程已停止")
            except:
                try:
                    self._blind_box_worker.terminate()
                    logger.log("  ✅ 盲盒血脉线程已强制终止")
                except:
                    pass
            self._blind_box_worker = None
    ...
    def _create_select_view(self):
        ...
        self.counter_season_combo.addItems(["第一赛季", "第二赛季"])
        self.counter_season_combo.setCurrentText("第二赛季")
    ...
    def _populate_pokemon_grid(self):
        ...
        season = self.counter_season_combo.currentText() if hasattr(self, 'counter_season_combo') else "第二赛季"
    ...
    def _create_pokemon_card(self, pokemon):
        ...
        if not image_loaded:
            # 获取当前选择的赛季（优先级：计数器界面 > 图鉴界面 > 默认第二赛季）
            season = "第二赛季"
    ...
    def _create_counter_list_item(self, counter):
        ...
        if not image_loaded:
            ...
            # 如果当前赛季没有，尝试从另一个赛季目录加载
            if not os.path.exists(image_path):
                other_season = "第二赛季" if season == "第一赛季" else "第一赛季"
                other_dir = os.path.join(self._base_dir, "image", "ys", other_season)
    ...
    def on_counter_context_menu(self, pos):
        ...
        export_action = menu.addAction("导出盲盒事件统计")
    ...
    def export_battle_stats(self, counter_id):
        ...
        if not counter.battle_pokemon_stats:
            QMessageBox.information(self, "提示", "当前没有盲盒事件期间的精灵统计数据")
            return
        ...
        # 创建导出格式选择对话框
        dialog = QDialog(self)
        dialog.setWindowTitle("导出盲盒事件统计")
        ...
        dlg = QFileDialog(self, "选择保存位置", f"{counter.pokemon_name}_盲盒事件统计")
        ...
        if base_path and (base_path.endswith('/') or base_path.endswith('\\')):
            # 如果被解释为目录，使用默认文件名
            base_path = os.path.join(base_path, f"{counter.pokemon_name}_盲盒事件统计")
...
<head>
    ...
    <title>盲盒事件统计 - {counter.pokemon_name}</title>
...
<body>
    <div class="container">
        <h1>{counter.pokemon_name} - 盲盒事件统计报告</h1>
...
'''
    ...
    def on_reset_counter(self):
        ...
        if reply == QMessageBox.Yes:
            active.count = 0
            active.battle_pokemon_stats = {}  # 清空盲盒事件精灵统计
    ...
    def load_settings_to_ui(self):
        ...
        # 盲盒设置
        if hasattr(self, 'blind_box_detection_switch'):
            self.blind_box_detection_switch.setChecked(self.settings_manager.get("enable_blind_box_detection", False))
        if hasattr(self, 'blind_box_confidence_spin'):
            self.blind_box_confidence_spin.setValue(self.settings_manager.get("blind_box_confidence", 0.6))
        if hasattr(self, 'mx_confidence_spin'):
            self.mx_confidence_spin.setValue(self.settings_manager.get("mx_confidence", 0.15))
        if hasattr(self, 'blind_box_interval_spin'):
            self.blind_box_interval_spin.setValue(self.settings_manager.get("blind_box_interval", 5))
        if hasattr(self, 'blind_box_roi_label'):
            blind_box_roi = self.settings_manager.get("blind_box_roi")
            if blind_box_roi and all(k in blind_box_roi for k in ("x", "y", "width", "height")):
                self.blind_box_roi_label.setText(
                    f"当前盲盒框选区域: x={blind_box_roi['x']}, y={blind_box_roi['y']}, w={blind_box_roi['width']}, h={blind_box_roi['height']}"
                )
            else:
                self.blind_box_roi_label.setText("当前未设置盲盒框选区域")
    ...
    def save_ui_to_settings(self):
        ...
        # 盲盒设置
        if hasattr(self, 'blind_box_detection_switch'):
            self.settings_manager.set("enable_blind_box_detection", self.blind_box_detection_switch.isChecked())
        if hasattr(self, 'blind_box_confidence_spin'):
            self.settings_manager.set("blind_box_confidence", self.blind_box_confidence_spin.value())
        if hasattr(self, 'mx_confidence_spin'):
            self.settings_manager.set("mx_confidence", self.mx_confidence_spin.value())
        if hasattr(self, 'blind_box_interval_spin'):
            self.settings_manager.set("blind_box_interval", self.blind_box_interval_spin.value())
    ...
    def on_blind_box_select(self):
        ...
        def on_region_selected(x, y, w, h):
            nonlocal finished
            self.settings_manager.set("blind_box_roi", {"x": x, "y": y, "width": w, "height": h})
            self.settings_manager.save_settings()

            # 更新UI显示
            if hasattr(self, 'blind_box_roi_label'):
                self.blind_box_roi_label.setText(
                    f"当前盲盒框选区域: x={x}, y={y}, w={w}, h={h}"
                )

            # 自动截取框选区域并保存
            try:
                roi_image = self.game_capture.capture_window((x, y, w, h))
                if roi_image is not None and roi_image.size > 0:
                    # 保存到image文件夹
                    image_dir = os.path.join(os.path.dirname(__file__), '..', 'image')
                    if not os.path.exists(image_dir):
                        os.makedirs(image_dir)
                    save_path = os.path.join(image_dir, "blind_box_roi_preview.png")
                    _imwrite(save_path, roi_image)
                    logger.log(f"✅ 盲盒框选区域已保存: {save_path}")
            except Exception as e:
                logger.log(f"⚠️ 保存盲盒框选区域失败: {e}")
    ...
    def _load_blind_box_roi(self):
        """从设置加载盲盒框选区域"""
        saved_roi = self.settings_manager.get("blind_box_roi")
        if saved_roi and all(k in saved_roi for k in ("x", "y", "width", "height")):
            self.blind_box_roi = (saved_roi["x"], saved_roi["y"], saved_roi["width"], saved_roi["height"])
            if hasattr(self, 'blind_box_roi_label'):
                roi = self.blind_box_roi
                self.blind_box_roi_label.setText(
                    f"当前盲盒框选区域: x={roi[0]}, y={roi[1]}, w={roi[2]}, h={roi[3]}"
                )
            print(f"已加载盲盒框选区域: {self.blind_box_roi}")
        else:
            self.blind_box_roi = None
    ...
    def create_counter_from_pokemon(self, pokemon):
        ...
        if global_count > 0:
            ...
            reply = QMessageBox.question(
                self,
                "同步数据",
                f"检测到【{name}】已有 {global_count} 次盲盒事件记录。\n\n"
                f"是否将这些记录同步到新创建的计数器？",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.Yes
            )
    ...
    def _create_settings_view(self):
        ...
        ocr_section = self._create_clean_section(
            ...
            [
                ...
                ("盲盒事件置信度", "盲盒事件模板的识别阈值，越高越准确", self.confidence_pollution_spin),
        ...
        # 盲盒框选设置
        blind_box_section = QWidget()
        blind_box_section_layout = QVBoxLayout(blind_box_section)
        blind_box_section_layout.setContentsMargins(0, 0, 0, 0)
        blind_box_section_layout.setSpacing(16)

        blind_box_title = QLabel("盲盒框选设置")
        blind_box_title.setStyleSheet("color: #ffffff; font-size: 18px; font-weight: 600;")
        blind_box_section_layout.addWidget(blind_box_title)

        blind_box_card = QFrame()
        blind_box_card.setStyleSheet("""
            QFrame {
                background-color: #1e1e26;
                border: none;
                border-radius: 8px;
        ...
        """)
        blind_box_card_layout = QVBoxLayout(blind_box_card)
        blind_box_card_layout.setContentsMargins(24, 18, 24, 18)
        blind_box_card_layout.setSpacing(12)

        # 盲盒血脉识别开关
        blind_box_toggle_row = QHBoxLayout()
        blind_box_toggle_row.setContentsMargins(0, 0, 0, 0)

        blind_box_toggle_info = QVBoxLayout()
        blind_box_toggle_info.setContentsMargins(0, 0, 0, 0)
        blind_box_toggle_info.setSpacing(2)

        blind_box_toggle_title = QLabel("启用盲盒血脉识别")
        blind_box_toggle_title.setStyleSheet("color: #e2e8f0; font-size: 14px; font-weight: 500;")

        blind_box_toggle_desc = QLabel("开启后，6秒空窗期自动识别盲盒区域的奇异/污染血脉")
        blind_box_toggle_desc.setStyleSheet("color: #71717a; font-size: 12px;")
        blind_box_toggle_desc.setWordWrap(True)

        blind_box_toggle_info.addWidget(blind_box_toggle_title)
        blind_box_toggle_info.addWidget(blind_box_toggle_desc)
        blind_box_toggle_row.addLayout(blind_box_toggle_info)
        blind_box_toggle_row.addStretch()

        self.blind_box_detection_switch = ToggleSwitch()
        blind_box_toggle_row.addWidget(self.blind_box_detection_switch)

        blind_box_card_layout.addLayout(blind_box_toggle_row)

        # 分隔线
        separator = QFrame()
        separator.setFrameShape(QFrame.HLine)
        separator.setStyleSheet("background-color: #2a2a35; max-height: 1px;")
        blind_box_card_layout.addWidget(separator)

        # 置信度设置
        confidence_row = QHBoxLayout()
        confidence_row.setContentsMargins(0, 0, 0, 0)

        confidence_label = QLabel("盲盒识别置信度")
        confidence_label.setStyleSheet("color: #e2e8f0; font-size: 13px;")
        confidence_row.addWidget(confidence_label)

        confidence_row.addStretch()

        self.blind_box_confidence_spin = self._create_spin_input(0.3, 1.0, 0.6, "", 0.05, True)
        confidence_row.addWidget(self.blind_box_confidence_spin)

        blind_box_card_layout.addLayout(confidence_row)

        # MX模型置信度设置
        mx_confidence_row = QHBoxLayout()
        mx_confidence_row.setContentsMargins(0, 0, 0, 0)

        mx_confidence_label = QLabel("血脉加成识别置信度")
        mx_confidence_label.setStyleSheet("color: #e2e8f0; font-size: 13px;")
        mx_confidence_row.addWidget(mx_confidence_label)

        mx_confidence_row.addStretch()

        self.mx_confidence_spin = self._create_spin_input(0.01, 1.0, 0.15, "", 0.05, True)
        mx_confidence_row.addWidget(self.mx_confidence_spin)

        blind_box_card_layout.addLayout(mx_confidence_row)

        # 检测间隔设置
        interval_row = QHBoxLayout()
        interval_row.setContentsMargins(0, 0, 0, 0)

        interval_label = QLabel("盲盒检测帧间隔")
        interval_label.setStyleSheet("color: #e2e8f0; font-size: 13px;")
        interval_row.addWidget(interval_label)

        interval_row.addStretch()

        self.blind_box_interval_spin = self._create_spin_input(1, 60, 5, " 帧")
        interval_row.addWidget(self.blind_box_interval_spin)

        blind_box_card_layout.addLayout(interval_row)

        # 分隔线
        separator2 = QFrame()
        separator2.setFrameShape(QFrame.HLine)
        separator2.setStyleSheet("background-color: #2a2a35; max-height: 1px;")
        blind_box_card_layout.addWidget(separator2)

        blind_box_btn = QPushButton("盲盒框选")
        blind_box_btn.setFixedHeight(44)
        blind_box_btn.setCursor(Qt.PointingHandCursor)
        blind_box_btn.setStyleSheet("""
            QPushButton {
                background-color: #7c3aed;
                color: #ffffff;
                border: 2px solid #a855f7;
        ...
        """)
        blind_box_btn.clicked.connect(self.on_blind_box_select)

        blind_box_info = QLabel("框选盲盒区域，程序启动时可自动定位到该区域进行识别")
        blind_box_info.setStyleSheet("color: #71717a; font-size: 13px;")
        blind_box_info.setWordWrap(True)

        # 显示当前盲盒框选区域
        self.blind_box_roi_label = QLabel("当前未设置盲盒框选区域")
        self.blind_box_roi_label.setStyleSheet("color: #e2e8f0; font-size: 13px; padding: 12px; background-color: #252530; border-radius: 6px;")

        blind_box_card_layout.addWidget(blind_box_info)
        blind_box_card_layout.addWidget(blind_box_btn)
        blind_box_card_layout.addWidget(self.blind_box_roi_label)

        blind_box_section_layout.addWidget(blind_box_card)
        content_layout.addWidget(blind_box_section)
        ...
        hotkey_functions = [
            ...
            ("counter_prev", "上一个计数器", "["),
            ("counter_next", "下一个计数器", "]"),
            ("nightmare_plus", "盲盒提示 +1", "》"),
            ("nightmare_minus", "盲盒提示 -1", "《"),
        ]
    ...
    def _create_combo_box(self, items, current_value):
        ...
        prob_desc = QLabel("用于计算盲盒事件概率的基础值（百分比）")
    ...
    def _create_pokedex_view(self):
        ...
        self.season_combo.addItems(["第一赛季", "第二赛季"])
        self.season_combo.setCurrentText("第二赛季")
    ...
    def _create_pokedex_item(self, pokemon):
        ...
        if not image_loaded:
            ...
            if hasattr(self, 'season_combo'):
                ...
                # 计数器界面可能没有season_combo，直接从第二赛季加载
                image_dir = os.path.join(self._base_dir, "image", "ys", "第二赛季")
                image_path = os.path.join(image_dir, f"{pokemon_name}.png")
    ...
    def _create_map_view(self):
        ...
        self.map_resource_btn.setFixedWidth(150)
        ...
        self.owl_stars_btn.setFixedWidth(150)
        ...
        """)
        zoom_in_btn.clicked.connect(lambda: self._zoom_map(1.2, None))
        ...
        """)
        zoom_out_btn.clicked.connect(lambda: self._zoom_map(0.8, None))
        ...
        collect_file = os.path.join(os.path.dirname(__file__), '..', 'collect_resources_final.json')
        if os.path.exists(collect_file):
            try:
                with open(collect_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.collect_data = data.get('resources', {})
    ...
    def _update_coordinate_display(self, map_x, map_y):
        """更新坐标显示"""
        # 将地图坐标转换为游戏坐标
        LAT_MIN, LAT_MAX = -2098, 1829
        LNG_MIN, LNG_MAX = -3015, 2799
        
        map_width = getattr(self, 'map_width', 1158)
        map_height = getattr(self, 'map_height', 1204)
        
        lat_range = LAT_MAX - LAT_MIN
        lng_range = LNG_MAX - LNG_MIN
        
        scale_x = map_width / lng_range
        scale_y = map_height / lat_range
        
        # 反向转换：地图坐标 -> 游戏坐标
        lng = map_x / scale_x + LNG_MIN
        lat = LAT_MAX - (map_y / scale_y)
        
        # 更新标签
        self.coord_label.setText(f"地图: ({map_x:.0f}, {map_y:.0f}) | 游戏: ({lat:.0f}, {lng:.0f})")
    ...
    def _start_navigation(self):
        ...
        if hasattr(self, 'current_region'):
            if self.current_region == 'full':
                map_path = os.path.join(os.path.dirname(__file__), '..', 'image', 'map_full_hq_resized.png')
            else:
                map_path = os.path.join(os.path.dirname(__file__), '..', 'image', f'map_{self.current_region}.png')
            
            print(f"文件是否存在: {os.path.exists(map_path)}")
    ...
    def _game_to_map_coords(self, lat, lng):
        ...
        Returns:
            (x, y): 地图像素坐标
        """
        # 根据用户提供的测试数据精确计算坐标转换参数
        # 第一组测试数据：
        # lat: 958, lng: -1899 -> 期望地图: (296, 1009)
        # lat: 999, lng: -1827 -> 期望地图: (311, 1018)
        # lat: 973, lng: -1806 -> 期望地图: (318, 1011)
        
        # 第二组测试数据：
        # lat: 1219, lng: -677 -> 期望地图: (599, 1075)
        # lat: 1233, lng: -641 -> 期望地图: (607, 1078)
        # lat: 1379, lng: -606 -> 期望地图: (617, 1114)
        # lat: 1460, lng: -387 -> 期望地图: (671, 1134)
        
        # 使用蓝晶碧玺和黄石榴石数据精确计算坐标转换
        
        # 测试数据（基于原始高清地图 12288x9216）：
        # 蓝晶碧玺: (lat=-1626, lng=-2927) -> (x=38, y=367)
        # 黄石榴石: (lat=958, lng=-1899) -> (x=296, y=1009)
        
        # 根据实际测试数据计算的坐标转换参数（针对 4096x3072 高清地图）
        scale_x = 0.669147
        offset_x = 2054.506880
        scale_y = 0.663536
        offset_y = 1540.584335
    ...
    def _load_map_image(self, region):
        ...
        if region == 'full':
            # 使用高清完整地图（缩放版，避免Qt加载大图片失败）
            map_path = os.path.join(base_dir, 'image', 'map_full_hq_resized.png')
            
            if not os.path.exists(map_path):
                logger.log(f"⚠️ G层高清地图文件不存在: {map_path}")
                return
            ...
            if self.map_pixmap.isNull():
                logger.log(f"⚠️ G层高清地图加载失败: {map_path}")
                return
            ...
            # B1/B2 层使用新拼接的地图
            map_path = os.path.join(base_dir, 'image', f'map_full_{region}.png')
            ...
            # 其他区域仍使用瓦片拼接（保留旧逻辑）
            tiles_dir = os.path.join(base_dir, 'image', 'map_tiles_complete')
    ...
    def _zoom_map(self, factor, mouse_pos=None):
        ...
        if hasattr(self, 'map_width') and hasattr(self, 'map_height'):
            new_width = int(self.map_width * self.map_scale)
            new_height = int(self.map_height * self.map_scale)
            self.map_label.setFixedSize(new_width, new_height)
    ...
    def _get_pokemon_image_path(self, pokemon_name):
        """获取精灵图片路径（支持赛季目录）"""
        # 先尝试从当前赛季目录加载
        season = self.season_combo.currentText() if hasattr(self, 'season_combo') else "第二赛季"
