...
class FloatingWindow(QWidget):
    ...
    nightmare_count_changed = Signal(int)  # 盲盒事件提示计数变化
    breakthrough_count_changed = Signal(int)  # 盲盒事件计数变化（幸运惊喜盒）
    ...
    def _init_ui(self):
        ...
        # 盲盒血脉显示标签（奇异/污染）- 始终可见，空文本时不可见
        self.blind_box_bloodline_label = QLabel("")
        self.blind_box_bloodline_label.setFixedWidth(40)
        self.blind_box_bloodline_label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        self.blind_box_bloodline_label.setStyleSheet("color: #fbbf24; font-size: 13px; font-weight: bold; margin-top: 4px;")
        current_lkwg_row.addWidget(self.blind_box_bloodline_label)
        ...
        # 盲盒事件提示（nightmare_template检测）
        self.nightmare_label = QLabel("盲盒事件提示: 0")
    ...
    def update_data(self, pokemon_name, type_, count, target, is_locked=False, nightmare_count=0, icon_id=0):
        ...
        self.nightmare_label.setText(f"盲盒事件提示: {nightmare_count}")
    ...
    def update_blind_box_bloodline(self, bloodline_type, bonus_type=None):
        """更新盲盒血脉显示（血脉+加成）
        血脉: 奇异/污染/混血/异色/普通
        加成: 物攻/魔攻/物防/魔防/速度/生命
        """
        if bloodline_type:
            color_map = {
                "奇异": "#a855f7",
            ...
            color = color_map.get(bloodline_type, "#fbbf24")
            self.blind_box_bloodline_label.setStyleSheet(f"color: {color}; font-size: 12px; font-weight: bold; margin-top: 4px;")
            
            text = bloodline_type
            if bonus_type:
                text = f"{bloodline_type} {bonus_type}"
            self.blind_box_bloodline_label.setText(text)
            self.blind_box_bloodline_label.setMaximumWidth(120)
            self.blind_box_bloodline_label.setMinimumWidth(0)
        else:
            self.blind_box_bloodline_label.setText("")
            self.blind_box_bloodline_label.setMaximumWidth(0)
            self.blind_box_bloodline_label.setMinimumWidth(0)
        self.blind_box_bloodline_label.repaint()
    ...
    def _load_pokemon_icon(self, pokemon_name, icon_id=0):
        ...
        if not image_loaded:
            ...
            # 如果赛季目录没有，尝试从另一个赛季目录加载
            if not os.path.exists(image_path):
                other_season = "第二赛季" if season == "第一赛季" else "第一赛季"
                other_dir = os.path.join(base_dir, "image", "ys", other_season)
    ...
    def update_nightmare_count(self, count):
        ...
        self.nightmare_label.setText(f"盲盒事件提示: {count}")
