...
from .blind_box_detector import BlindBoxDetector, BloodlineDetector
...
class GameCapture:
    def __init__(self, window_title="洛克王国：世界"):
        ...
        self.nl_trigger_timeout = 2.0  # 2秒触发超时（nl检测到后等待幸运惊喜盒）
        self.nl_was_detected = False  # nl是否曾经被检测到（用于防止重复触发）
        self.nl_detection_failed = False  # nl触发是否失败（2秒内未检测到幸运惊喜盒）
        self.battle_started = False  # 战斗是否已正式开始（检测到幸运惊喜盒后）
        ...
        # 盲盒血脉识别模板
        self.qy_template = None  # qy.png 奇异模板
        self.wr_template = None  # wr.png 污染模板
        self._qy_scaled_templates = {}  # qy多尺寸缓存
        self._wr_scaled_templates = {}  # wr多尺寸缓存
        self._qy_preferred_scale = None  # qy上次匹配成功的最佳缩放
        self._wr_preferred_scale = None  # wr上次匹配成功的最佳缩放
        ...
        # 盲盒检测器 (ORB特征匹配+单应性变换)
        self.blind_box_detector = None
        
        # 血脉+加成检测器 (ONNX Runtime)
        self.bloodline_detector = None
        self.mx_names = ['普通', '奇异', '异色', '混血', '污染',
                         '生命', '速度', '物攻', '魔攻', '物防', '魔防']
    ...
    def _load_templates(self):
        ...
        # 加载qy.png - 奇异模板
        qy_path = os.path.join(image_dir, "qy.png")
        if os.path.exists(qy_path):
            self.qy_template = _cv2_imread(qy_path, cv2.IMREAD_COLOR)
            if self.qy_template is not None:
                h, w = self.qy_template.shape[:2]
                self._qy_scaled_templates = {}
                self._qy_preferred_scale = None
                logger.log(f"✅ 已缓存qy.png模板 (原始尺寸: {w}x{h})")
            else:
                logger.log(f"⚠️ qy.png加载失败")
        else:
            logger.log(f"⚠️ qy.png不存在")

        # 加载wr.png - 污染模板
        wr_path = os.path.join(image_dir, "wr.png")
        if os.path.exists(wr_path):
            self.wr_template = _cv2_imread(wr_path, cv2.IMREAD_COLOR)
            if self.wr_template is not None:
                h, w = self.wr_template.shape[:2]
                self._wr_scaled_templates = {}
                self._wr_preferred_scale = None
                logger.log(f"✅ 已缓存wr.png模板 (原始尺寸: {w}x{h})")
            else:
                logger.log(f"⚠️ wr.png加载失败")
        else:
            logger.log(f"⚠️ wr.png不存在")

        # 初始化盲盒检测器 (YOLOv8n)
        try:
            self.blind_box_detector = BlindBoxDetector()
            if self.blind_box_detector.is_ready:
                logger.log(f"✅ YOLO盲盒检测器初始化成功")
            else:
                logger.log(f"⚠️ YOLO模型未加载，盲盒检测不可用")
                self.blind_box_detector = None
        except Exception as e:
            logger.log(f"⚠️ 盲盒检测器初始化失败: {e}")
            self.blind_box_detector = None

        # 初始化血脉+加成检测器 (ONNX Runtime — 跨平台一致)
        mx_confidence = getattr(self, 'settings_manager', None) and self.settings_manager.get("mx_confidence", 0.15) or 0.15
        try:
            base_path = os.path.dirname(os.path.dirname(__file__))
            onnx_path = os.path.join(base_path, 'runs', 'mx_yolo26n', 'weights', 'best.onnx')
            self.bloodline_detector = BloodlineDetector(onnx_path, conf_threshold=mx_confidence)
            if self.bloodline_detector.is_ready:
                logger.log(f"✅ 血脉加成检测器初始化成功 (ONNX Runtime)")
            else:
                logger.log(f"⚠️ 血脉加成检测器初始化失败，血脉识别不可用")
        except Exception as e:
            logger.log(f"⚠️ 血脉加成检测器初始化异常: {e}")
            self.bloodline_detector = None
    ...
    def get_model_status(self):
        ...
        lines = []
        lines.append("📦 模型加载状态:")
        if self.blind_box_detector is not None:
            lines.append("  " + self.blind_box_detector.get_status())
        else:
            lines.append("  ❌ 盲盒检测器未初始化")
        if self.bloodline_detector is not None:
            lines.append("  " + self.bloodline_detector.get_status())
        else:
            lines.append("  ❌ 血脉加成检测器未初始化")
        return "\n".join(lines)
    ...
    def _match_pokemon_names(self, texts):
        ...
        for text in texts:
            ...
            # 特殊处理："幸运惊喜盒"直接通过，不经过进化链匹配
            if text_clean == "幸运惊喜盒":
                matched.append("幸运惊喜盒")
                continue
    ...
    def _detect_blind_box_grid(self, image):
        """使用 YOLO 检测盲盒位置。"""
        if image is None:
            return {'detected': False, 'score': 0}
        if self.blind_box_detector is not None:
            result = self.blind_box_detector.detect(image)
            if result.get('detected'):
                b = result.get('box', {})
                logger.log(f"🔍 YOLO盲盒检测: 置信度={result.get('score',0):.1f}"
                           f" 位置=({b.get('x',0)},{b.get('y',0)})~({b.get('x',0)+b.get('w',0)},{b.get('y',0)+b.get('h',0)})")
            return result
        return {'detected': False, 'score': 0}

    def _detect_bloodline_bonus(self, image, box, l2, l3):
        """使用MX附属模型识别血脉和加成
        流程: 裁剪505x620区域 -> MX模型 -> L2/L3 ROI过滤
        :return: (bloodline_name, bloodline_conf, bonus_name, bonus_conf) 或 (None,0,None,0)
        """
        if self.bloodline_detector is None or not self.bloodline_detector.is_ready:
        ...
        # 基于盲盒位置裁剪505x620区域（匹配训练数据）
        OFF_X, OFF_Y = 204, 22
        CROP_W, CROP_H = 505, 620
        
        cx1 = max(0, box['x'] - OFF_X)
        cy1 = max(0, box['y'] + OFF_Y)
        ...
        # 保存裁剪图片到image目录（每次都覆盖）
        try:
            debug_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "image")
            os.makedirs(debug_dir, exist_ok=True)
            debug_path = os.path.join(debug_dir, "debug_xm.jpg")
            _cv2_imwrite(debug_path, crop.copy())
        except Exception:
            pass
        
        # 映射L2/L3到裁剪坐标
        l2_x1 = max(0, l2['x'] - cx1)
        l2_y1 = max(0, l2['y'] - cy1)
        l2_x2 = min(cw, l2_x1 + l2['w'])
        l2_y2 = min(ch, l2_y1 + l2['h'])
        
        l3_x1 = max(0, l3['x'] - cx1)
        l3_y1 = max(0, l3['y'] - cy1)
        l3_x2 = min(cw, l3_x1 + l3['w'])
        l3_y2 = min(ch, l3_y1 + l3['h'])
        
        # ONNX Runtime 推理（跨平台一致）
        mx_confidence = self.settings_manager.get("mx_confidence", 0.15) if hasattr(self, 'settings_manager') else 0.15
        self.bloodline_detector.conf_threshold = mx_confidence
        detections = self.bloodline_detector.detect(crop)
        ...
        for det in detections:
            ...
            det_cx = (xyxy[0] + xyxy[2]) / 2
            det_cy = (xyxy[1] + xyxy[3]) / 2
            
            in_l2 = l2_x1 <= det_cx <= l2_x2 and l2_y1 <= det_cy <= l2_y2
            in_l3 = l3_x1 <= det_cx <= l3_x2 and l3_y1 <= det_cy <= l3_y2
            
            if in_l2 and cls_id <= 4:
                bloodline_cands.append((self.mx_names[cls_id], conf, xyxy))
            elif in_l3 and cls_id >= 5:
                bonus_cands.append((self.mx_names[cls_id], conf, xyxy))
        
        bloodline_cands.sort(key=lambda x: x[1], reverse=True)
        bonus_cands.sort(key=lambda x: x[1], reverse=True)
        
        bl_name = bloodline_cands[0][0] if bloodline_cands else None
        bl_conf = bloodline_cands[0][1] if bloodline_cands else 0
        bn_name = bonus_cands[0][0] if bonus_cands else None
        bn_conf = bonus_cands[0][1] if bonus_cands else 0
        
        return bl_name, bl_conf, bn_name, bn_conf

    def _match_bloodline_label(self, image, attempt=1):
        roi_h, roi_w = image.shape[:2]
        if roi_w <= 0 or roi_h <= 0:
            return None

        threshold = self.settings_manager.get("blind_box_confidence", 0.6) if hasattr(self, 'settings_manager') else 0.6
        all_scales = [0.8, 0.9, 1.0, 1.1, 1.2]
        ...
        img_proc = _preprocess(image)

        def _try_scale(template, scaled_cache, scale):
            h, w = template.shape[:2]
            new_w = int(w * scale)
            new_h = int(h * scale)
            if new_w > roi_w or new_h > roi_h or new_w < 5 or new_h < 5:
                return None
            cache_key = f"{new_w}x{new_h}"
            if cache_key not in scaled_cache:
                scaled = cv2.resize(template, (new_w, new_h))
                scaled_cache[cache_key] = _preprocess(scaled)
            scaled_proc = scaled_cache[cache_key]
            result = cv2.matchTemplate(img_proc, scaled_proc, cv2.TM_CCOEFF_NORMED)
            _, max_val, _, _ = cv2.minMaxLoc(result)
            return max_val

        # 检测 qy
        if self.qy_template is not None:
            first_scale = self._qy_preferred_scale if self._qy_preferred_scale is not None else 1.0
            score = _try_scale(self.qy_template, self._qy_scaled_templates, first_scale)
            if score is not None and score >= threshold:
                self._qy_preferred_scale = first_scale
                logger.log(f"✅ 血脉匹配: 奇异 (置信度={score:.3f})")
                return "奇异"

            best_qy = score if score is not None else 0
            for scale in all_scales:
                if scale == first_scale:
                    continue
                score = _try_scale(self.qy_template, self._qy_scaled_templates, scale)
                if score is not None and score > best_qy:
                    best_qy = score
                if score is not None and score >= threshold:
                    self._qy_preferred_scale = scale
                    logger.log(f"✅ 血脉匹配: 奇异 (置信度={score:.3f})")
                    return "奇异"
        else:
            best_qy = 0
        ...
        # 检测 wr
        if self.wr_template is not None:
            first_scale = self._wr_preferred_scale if self._wr_preferred_scale is not None else 1.0
            score = _try_scale(self.wr_template, self._wr_scaled_templates, first_scale)
            if score is not None and score >= threshold:
                self._wr_preferred_scale = first_scale
                logger.log(f"✅ 血脉匹配: 污染 (置信度={score:.3f})")
                return "污染"

            best_wr = score if score is not None else 0
            for scale in all_scales:
                if scale == first_scale:
                    continue
                score = _try_scale(self.wr_template, self._wr_scaled_templates, scale)
                if score is not None and score > best_wr:
                    best_wr = score
                if score is not None and score >= threshold:
                    self._wr_preferred_scale = scale
                    logger.log(f"✅ 血脉匹配: 污染 (置信度={score:.3f})")
                    return "污染"
        else:
            best_wr = 0
        ...
        # 只在首次尝试失败时输出日志
        if attempt <= 1:
            logger.log(f"ℹ️ 血脉标签未匹配 - 最佳: 奇异={best_qy:.3f}, 污染={best_wr:.3f}, 阈值={threshold}")
        return None
        ...
        if self.nightmare_cached_template is not None:
            ...
            if detected:
                ...
                if current_time - self.last_nightmare_time >= self.nightmare_cooldown:
                    ...
                    logger.log(f"✅ 检测到nightmare_template(缓存)，盲盒事件数: {self.nightmare_detected_count}")
                else:
        ...
        for scale in scales:
            ...
            if len(loc[0]) > 0:
                ...
                if current_time - self.last_nightmare_time >= self.nightmare_cooldown:
                    self.nightmare_detected_count += 1
                    self.last_nightmare_time = current_time
                    logger.log(f"✅ 检测到nightmare_template: 缩放={scale}x, 盲盒事件数: {self.nightmare_detected_count}")
                # 冷却中不输出日志,避免刷屏
                break
    ...
    def reset_nightmare_count(self):
        """重置盲盒事件提示计数"""
        self.nightmare_detected_count = 0
        logger.log(f"🔄 盲盒事件提示计数已重置为 0")
    
    def set_nightmare_count(self, count):
        """设置盲盒事件提示计数"""
        self.nightmare_detected_count = max(0, count)
        logger.log(f"🔄 盲盒事件提示计数已设置为 {self.nightmare_detected_count}")
    
    def detect_nl_trigger(self, image=None):
        """
        检测是否出现nl.png模板（盲盒事件触发标志，带多尺度缩放和智能缓存）
        :param image: 截图图像，如果为None则自动捕获
        :return: True/False
        """
        if image is None:
        ...
        if self.ocr_enabled:
            ...
            if nl_detected:
                if not self.nl_was_detected:
                    ...
                    self.nl_detection_failed = False
                    logger.log("🚀 检测到nl模板，启动OCR，等待2秒内检测幸运惊喜盒")
    ...
    def update_ocr_state(self, has_valid_recognition, recognized_names=None):
        ...
        if self.ocr_enabled:
            # 阶段1：nl触发后2秒内必须检测到“幸运惊喜盒”
            if not self.battle_started and self.nl_trigger_time > 0:
                elapsed_since_nl = current_time - self.nl_trigger_time
                
                # 检查是否已经检测到幸运惊喜盒
                has_lucky_box = recognized_names and "幸运惊喜盒" in recognized_names
                
                if has_lucky_box:
                    # 检测到幸运惊喜盒，战斗正式开始
                    logger.log("✅ nl触发后检测到幸运惊喜盒，战斗开始")
                    ...
                    self.nl_was_detected = False  # 重置nl触发状态，允许下次触发
                    logger.log(f"⏱️ nl触发后{elapsed_since_nl:.1f}秒内未检测到幸运惊喜盒，关闭OCR，等待nl消失")
...
class BlindBoxBloodlineWorker(QThread):
    """盲盒血脉检测线程
    流程:
    1. YOLO 循环检测盲盒（独立帧间隔，不共享OCR延迟）
    2. 检测到 → 用MX附属模型识别血脉+加成
    3. 匹配成功 → emit bloodline_bonus_result 并退出
    """
    bloodline_result = Signal(str)
    bloodline_bonus_result = Signal(str, str, float, float)
    ...
    def configure(self, roi, confidence, timeout=5.0, detect_interval=5):
        self.roi = roi
    ...
    def run(self):
        ...
        logger.log(f"🔧 BlindBoxBloodlineWorker.run() 开始执行 (configured={self._configured})")

        if not self._configured:
            logger.log(f"⚠️ 盲盒检测线程未配置，退出")
            return
        ...
        if self.game_capture.blind_box_detector is None:
            logger.log("⚠️ 盲盒检测器未初始化")
            return
        ...
        # Phase 1: YOLO 循环检测盲盒（独立帧间隔，不依赖OCR延迟）
        interval = max(1, self.detect_interval)
        logger.log(f"🔍 YOLO盲盒检测启动: timeout={self.timeout}s, 每{interval}帧检测一次")

        frame_count = 0
        last_capture_time = 0
        min_capture_interval = 0.016  # 最小截图间隔 16ms，避免在同一帧上反复截图

        while self._running:
            elapsed = time.time() - start_time
            if elapsed > self.timeout:
                logger.log(f"⏱️ 盲盒检测超时({self.timeout:.0f}s), 共{frame_count}帧, {loop_count}次扫描均未检测到盲盒")
                return

            frame_count += 1
            try:
                # 跳过不检测的帧，连截图都不做。避免 capture_window 耗时导致帧间隔被无限拉长
                if frame_count % interval != 0:
                    self.msleep(16)
                    continue

                # 两次截图之间至少间隔 16ms，防止连续截到同一帧
                now = time.time()
                ...
                loop_count += 1
                result = self.game_capture._detect_blind_box_grid(image)
                if result.get('detected'):
                    elapsed_ms = (time.time() - start_time) * 1000
                    b = result.get('box', {})
                    l2 = result.get('layer2', {})
                    l3 = result.get('layer3', {})
                    logger.log(f"✅ YOLO检测到盲盒 (第{frame_count}帧/第{loop_count}次扫描, {elapsed_ms:.0f}ms, 置信度={result['score']:.1f}, "
                               f"位置=({b.get('x',0)},{b.get('y',0)})~({b.get('x',0)+b.get('w',0)},{b.get('y',0)+b.get('h',0)})"
                               f" L2=({l2.get('x','?')},{l2.get('y','?')}) L3=({l3.get('x','?')},{l3.get('y','?')})")

                    annotated = self.game_capture.blind_box_detector.draw_result(image, result)
                    _cv2_imwrite(os.path.join(image_dir, "blind_box_detected.png"), annotated)
                    logger.log(f"📸 盲盒截图已保存(带标注框): blind_box_detected.png")
                ...
                if loop_count % 5 == 1:
                    logger.log(f"⏳ 正在检测盲盒... (第{frame_count}帧, 已扫描{loop_count}次, {elapsed:.1f}s)")

                self.msleep(16)
            except Exception as e:
                logger.log(f"❌ 盲盒检测异常: {e}")
                self.msleep(100)

        if not detected_result:
            logger.log(f"⏱️ 盲盒检测结束：未检测到盲盒")
            return

        # Phase 2: 用MX模型识别血脉和加成
        self._match_bloodline_mx(detected_result, detected_image, image_dir, start_time)

    def _match_bloodline_mx(self, grid_result, detection_frame, image_dir, start_time):
        import time, os, cv2

        layer2 = grid_result.get('layer2')
        layer3 = grid_result.get('layer3')
        bb = grid_result.get('box')
        if not layer2 or not layer3 or not bb:
            logger.log("⚠️ 检测结果中无完整ROI区域，无法识别")
            return

        # 直接使用盲盒检测时的截图裁剪做血脉识别，不再单独截图
        bl_name, bl_conf, bn_name, bn_conf = self.game_capture._detect_bloodline_bonus(
            detection_frame, bb, layer2, layer3
        )

        if bl_name and bn_name:
            total_ms = (time.time() - start_time) * 1000
            logger.log(f"✅ MX血脉加成识别成功: 血脉={bl_name}({bl_conf:.0%}) 加成={bn_name}({bn_conf:.0%}) (总耗时{total_ms:.0f}ms)")
            self.bloodline_result.emit(bl_name)
            self.bloodline_bonus_result.emit(bl_name, bn_name, bl_conf, bn_conf)
            return

        # 退而求其次：只要有血脉或加成之一就报告
        if bl_name or bn_name:
            total_ms = (time.time() - start_time) * 1000
            logger.log(f"⚠️ MX部分识别: 血脉={bl_name or '?'}({bl_conf:.0%}) 加成={bn_name or '?'}({bn_conf:.0%}) (总耗时{total_ms:.0f}ms)")
            self.bloodline_result.emit(bl_name or '?')
            self.bloodline_bonus_result.emit(bl_name or '?', bn_name or '?', bl_conf, bn_conf)
            return

        logger.log(f"❌ MX识别失败: 未检测到任何血脉或加成")
