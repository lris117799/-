...
class SettingsManager:
    def __init__(self):
        ...
        "confidence_pollution": 0.6,  # 盲盒事件用的置信度
        ...
        "enable_background_recognition": True,
            ...
            # 盲盒框选区域
            "blind_box_roi": None,  # 盲盒框选区域 (x, y, width, height)
            
            # 盲盒血脉识别设置
            "enable_blind_box_detection": True,  # 盲盒开关：开启后6秒空窗期识别奇异/污染
            "blind_box_confidence": 0.6,  # 盲盒识别置信度 (0.0~1.0)
            "mx_confidence": 0.15,  # 附属模型血脉+加成识别置信度 (0.0~1.0)
            "blind_box_interval": 5,  # 盲盒检测帧间隔（每N帧检测一次）
